...
# 案由
def anyou_list(sql_reason):
    cursor.execute(sql_reason)
    row = cursor.fetchall()
    reason_list = []
    for i in row:
        # print i[0]
        reason_list.append(i[0])
    return list(set(reason_list))

sql_reason = "select new_name from tb_reason"
reason_list = anyou_list(sql_reason)

import json
import pandas as pd
sql = "select uuid,reason,punish_cate,punish_date,delay_date,punish_money,casedate,province from tmp_liufang"
cursor.execute(sql)
row = cursor.fetchall()

for k in reason_list:
    # 省份
    reason_province = []
    # 近10年案件数
    new10_dict = {}
    ...
    for i in row:
        if k in i[1]:
            ...
            # 省份
            if i[7] != '' and i[7] != 'None':
                reason_province.append(i[7])
    province_dict={}
    province_list = pd.Series(reason_province).value_counts()
    for n in range(len(province_list)):
        province_dict[province_list.keys()[n]]=int(province_list[n])
    # print k,province_dict.keys()
    ...
    province_dict = json.dumps(province_dict, ensure_ascii=False)
    value = [k,reason_num,province_dict,new10_dict,punish_cate_dict,punish_date_dict,delay_cate_dict,delay_date_dict,punish_money_dict]
    print k,reason_num,province_dict,new10_dict,punish_cate_dict,punish_date_dict,delay_cate_dict,delay_date_dict,punish_money_dict
#   插入数据
    insert_sql = "insert into scene_reason(reason,reason_num,province_dict,new10_dict,punish_cate_dict,punish_date_dict,delay_cate_dict,delay_date_dict,punish_money_dict) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
