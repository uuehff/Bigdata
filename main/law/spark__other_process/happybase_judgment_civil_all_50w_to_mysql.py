...
try:
    conn = pymysql.connect(host='cdh-slave1', user='weiwc', passwd='HHly2017.', db='laws_doc_v2', charset='utf8')
    cursor = conn.cursor()
    for k, v in q:
        # print k,v
        # data = {}
        # for k1 in v.keys():
            # data.update({k1:v[k1]})
        sql2 = " insert into judgment_civil_all_50w (id,uuid,party_info,trial_process,trial_request,court_find,court_idea,judge_result,doc_footer,caseid,title,court,court_uid,lawlist,law_id,casedate,reason_type,type,judge_type,reason,reason_uid,province,plt_claim,dft_rep,crs_exm) " \
