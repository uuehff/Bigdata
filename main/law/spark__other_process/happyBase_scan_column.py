...
import time
# q = t.scan(filter="QualifierFilter(!=, 'binary:uuid_old') AND KeyOnlyFilter()")  #不存在uuid_old字段的数据
# q = t.scan(filter="QualifierFilter(=, 'binary:uuid_old') AND KeyOnlyFilter()",limit=10)
q = t.scan(filter="QualifierFilter(=, 'binary:uuid_old')",limit=10)
a = 0
...
# 000028f8-b092-4b10-8bcc-a747cb70dac5 {'d:uuid': ''}
# 00002d69-fb43-48c7-b38e-86f66a22ea76 {'d:uuid': ''}
# 000035e1-3e09-4278-89a9-bb56b252b0b5 {'d:uuid': ''}

#  For example, this will result in three round-trips to the server (two batches with 1000 cells, and one with the remaining 400):
#
...
#
# try:
#     with t.batch(batch_size=10000) as t_batch:
#         for k,vs in q:
#             # print k,vs
#             # for k1 in vs.keys():
#                 # print k1 , vs[k1]
