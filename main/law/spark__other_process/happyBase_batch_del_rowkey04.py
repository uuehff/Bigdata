...
conn=pymysql.connect(host='cdh5-slave2',user='weiwc',passwd='HHly2017.',db='laws_doc_zhangye_v2',charset='utf8')
cursor = conn.cursor()

#分批次读取mysql进行删除
for i in range(0,2):
    sql = 'select uuid from uuid456 where id > ' + str(i*40000) + ' and id <= ' + str((i+1)*40000)   #LOCATE函数，包含||,返回大于0的数值。
    cursor.execute(sql)
