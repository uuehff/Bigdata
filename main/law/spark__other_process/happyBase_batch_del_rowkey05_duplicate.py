...
conn=pymysql.connect(host='cdh5-slave2',user='weiwc',passwd='HHly2017.',db='docs_count',charset='utf8')
cursor = conn.cursor()

#分批次读取mysql进行删除
for i in range(0,2):
    # sql = 'select uuid from administration_civil where id > ' + str(i*40000) + ' and id <= ' + str((i+1)*40000)   #LOCATE函数，包含||,返回大于0的数值。
    # sql = 'select uuid from administration_implement where id > ' + str(i*40000) + ' and id <= ' + str((i+1)*40000)   #LOCATE函数，包含||,返回大于0的数值。
    # sql = 'select uuid from implement_civil where id > ' + str(i*40000) + ' and id <= ' + str((i+1)*40000)   #LOCATE函数，包含||,返回大于0的数值。
    sql = 'select uuid from judgment_civil where id > ' + str(i*40000) + ' and id <= ' + str((i+1)*40000)   #LOCATE函数，包含||,返回大于0的数值。
    cursor.execute(sql)
    ...
    try:
        ...
        with t.batch(batch_size=1000) as t_batch: #这句是经典的语句，一般都这样使用，with加batch_size。
            for row in row_2:
