...
def get_X(ll):
    # for i in index_tag.keys():
    #     print type(index_tag[i]),index_tag[i]
    # print ll
    # print ll
    # print ll[1]
    # content = ll[1]
...
def p(i):
    print type(i), i[0],i[1],i[1][3].encode("utf-8")
    print "--------------------------------------"
...
if __name__ == "__main__":
    ...
    # df = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave2:3306/laws_doc', table='judgment',predicates=["id > 100000 and id <= 110000","id > 110000 and id <= 120000","id > 120000 and id <= 130000","id > 130000 and id <= 140000","id > 140000 and id <= 150000"],properties={"user":"root","password":"root"})
    ...
    # df.map(lambda x : format_html_to_line(x['doc_content'])).map(lambda x: get_X(x)).foreach(lambda x :p(x))
    # df.map(lambda x : format_html_to_line(x['doc_content'])).foreach(lambda x :p(x))
        # .\
    ...
    #
    # df.map(lambda x: format_html_to_line(x)).map(lambda x: get_X(x)).flatMap(lambda x: save_result_to_hbase(x)).saveAsNewAPIHadoopDataset(conf=conf, keyConverter=keyConv, valueConverter=valueConv)
    #
    #
    # sc.stop()