...
def foot_get(items):
    ...
    if footer_result:
        ...
        doc_footer = doc_footer.replace(u"审判长",u" 审判长  ").replace(u"代理审判员", u" 代理审判员")\
            ...
            .replace(u"二",u" 二").replace(u"院长", u" 院长  ")
...
def format_content(content):
    ...
    content_text = content_text.replace("<span litigantpart></span>", "0|0|0|11").replace("<span proceeding></span>","0|0|0|22") \
        .replace("<span argued></span>", "0|0|0|33") \
    ...
    caseid_result = re.findall(reg_caseid, ss)

    # if len(ss.split(u"（20")) == 2:
    #     content_dict['caseid'] = u"（20" + ss.split(u"（20")[1]
    if caseid_result:
        content_dict['caseid'] = caseid_result[0]
...
def doc_items(items):
    ...
    return (id,uuid,party_info,trial_process,trial_request,court_find,court_idea,judge_result,doc_footer,court,caseid)
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.10.22:3306/laws_doc', table='(select id,uuid,doc_content,title,lawlist,reason_type,type,judge_type from mediate ) tmp',column='id',lowerBound=1,upperBound=131607,numPartitions=10,properties={"user": "tzp", "password": "123456"})
    df2 = sqlContext.read.jdbc(url='jdbc:mysql://192.168.74.102:3306/laws_doc', table='(select id,name from court where name is not null ) tmp',column='id',lowerBound=1,upperBound=5000,numPartitions=1,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_mediate?useUnicode=true&characterEncoding=utf8', table='mediate_civil_etl_court2',properties={"user": "weiwc", "password": "HHly2017."})
