...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.12.34:3306/laws_doc_mediate', table='(select id,uuid,doc_content from mediate_valid ) tmp',column='id',lowerBound=1,upperBound=131607,numPartitions=1,properties={"user": "weiwc", "password": "HHly2017."})
    df2 = sqlContext.read.jdbc(url='jdbc:mysql://192.168.12.34:3306/laws_doc', table='(select id,name from court where name is not null ) tmp',column='id',lowerBound=1,upperBound=5000,numPartitions=1,properties={"user": "weiwc", "password": "HHly2017."})
