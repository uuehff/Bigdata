...
def doc_items(items):
    ...
    if not doc_content or not doc_content.startswith('<a type='):
        return None
    ...
    return (id,uuid,party_info,trial_process,trial_request,court_find,court_idea,judge_result,doc_footer,uuid_)
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.10.22:3306/laws_doc', table='(select id,uuid,doc_content from adjudication where id < 500 and doc_from = "wenshu" ) tmp',column='id',lowerBound=1,upperBound=500,numPartitions=1,properties={"user": "tzp", "password": "123456"})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_adjudication?useUnicode=true&characterEncoding=utf8', table='adjudication_civil_etl_wenshu_v2',properties={"user": "weiwc", "password": "HHly2017."})
