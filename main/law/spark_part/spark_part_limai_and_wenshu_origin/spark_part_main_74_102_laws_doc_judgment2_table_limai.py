...
def doc_items(items):
    # id, uuid, doc_content, caseid, title, reason_origin, court_origin, \
    # lawlist, casedate_origin, reason_type, type, judge_type
    ...
    return (id,uuid,party_info,trial_process,trial_request,court_find,court_idea,judge_result,doc_footer,
            uuid_,caseid,title,court,lawlist,casedate,reason_type,type,judge_type,reason)
# id, uuid, doc_content, caseid, title, reason_origin, court_origin, \
    # lawlist, casedate_origin, reason_type, type, judge_type
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.74.102:3306/laws_doc2', table='(select id,uuid,doc_content,caseid,title,reason_origin,court_origin,lawlist,casedate_origin,reason_type,type,judge_type from judgment2) tmp',column='id',lowerBound=1,upperBound=44765,numPartitions=20,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_judgment?useUnicode=true&characterEncoding=utf8', table='judgment2_etl_v2',properties={"user": "weiwc", "password": "HHly2017."})
