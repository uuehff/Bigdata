...
def foot_get(items):
    ...
    reg_footer = re.compile(ur'\n(?:\S{0,2}审判|\S{0,2}执行员|\S{0,2}院长).*(?:书记员|\S+年\S+月)\S+', re.S)
    items = items.replace("\\n", '\n')
...
def format_content(content,doc_from):
    """
    理脉网内容分段
    :param content:
    :return:
...
def doc_items(items):
    ...
    if not doc_content or (not doc_content.endswith('/div&gt;') and not doc_content.startswith('<a type=')):
        return None

    content_text,doc_text = format_content(doc_content,doc_from)
    # if not content_text:
    #     return None
    ...
    doc_footer = ""
    if content_text.get('doc_from', '') == "limai":
    ...
    # 对最终的court_idea做处理，包含有标签和无标签两种情况得到的court_idea。
    ...
    return (id,uuid,party_info,trial_process,trial_request,court_find,court_idea,judge_result,doc_footer,uuid_,caseid,title,court,lawlist,reason_type,type,judge_type)
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.10.22:3306/laws_doc', table='(select id,uuid,doc_content,caseid,title,court,lawlist,reason_type,type,judge_type,doc_from from adjudication ) tmp',column='id',lowerBound=1,upperBound=3879069,numPartitions=99,properties={"user": "tzp", "password": "123456"})
    ...
    lawyer_k_v = df.map(lambda x:x).map(lambda x:doc_items(x)).filter(lambda x:filter_(x)).cache()
    # lawyer_k_v.foreach(p)
    # print "len===========" + str(lawyer_k_v.count())

    lawyer_k_v_civil = lawyer_k_v.filter(lambda x:filter_civil(x[14]))
    lawyer_k_v_xingshi = lawyer_k_v.filter(lambda x:filter_xingshi(x[14]))
    ...
    f1.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_adjudication?useUnicode=true&characterEncoding=utf8', table='adjudication_civil_etl_v3',properties={"user": "weiwc", "password": "HHly2017."})
    # f.show()
    # , mode = "overwrite"
    # useUnicode = true & characterEncoding = utf8，指定写入mysql时的数据编码，否则会乱码。

    f2 = sqlContext.createDataFrame(lawyer_k_v_xingshi, schema=schema)
    f2.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_adjudication?useUnicode=true&characterEncoding=utf8',
                  table='adjudication_xingshi_etl_v3', properties={"user": "weiwc", "password": "HHly2017."})
