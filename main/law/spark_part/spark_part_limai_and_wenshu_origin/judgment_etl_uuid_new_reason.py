...
if __name__ == "__main__":
    ...
    reason_null = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc', table='judgment_etl_uuid_new_reason',column='id',lowerBound=0,upperBound=100000,numPartitions=28,properties={"user": "root", "password": "root"})
    ...
    c = reason_null.map(lambda x:(x[1],x[2])).filter(lambda x:filter_(x)).flatMapValues(lambda x:extact_reason(x)).map(lambda x:(x[1],x[0])).groupByKey().mapValues(lambda v:get_uuids(v))
    ...
    schema = StructType([StructField("uuid", StringType(), False),StructField("reason_uids", StringType(), False)])

    f = sqlContext.createDataFrame(reason_uids_result, schema=schema)
    # , mode = "overwrite"
    # useUnicode = true & characterEncoding = utf8，指定写入mysql时的数据编码，否则会乱码。
    f.write.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc?useUnicode=true&characterEncoding=utf8', table='judgment_etl_uuid_new_reason_result',properties={"user": "root", "password": "root"})
