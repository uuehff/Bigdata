...
sql = 'select id,uuid,lawlist from judgment2'
cursor.execute(sql)
...
for row in row_2 :
    ...
    if s1:
        ...
        sql2 = " insert into tmp_weiwenchao (id,uuid,lawlist) values (%s, %s, %s)"
        cursor.execute(sql2,(row[0],row[1],",".join(s1)))
        # print ",".join(s1)
    conn.commit()
