...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.74.113:3306/wenshu_gov', table='(select id,uuid,doc_content,is_crawl from judgment ) tmp',column='id',lowerBound=1,upperBound=8450135,numPartitions=99,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    lawyer_k_v = df.map(lambda x:doc_items(x,court_value)).filter(lambda x:filter_(x))
    ...
    f1 = sqlContext.createDataFrame(lawyer_k_v, schema=schema)
    ...
    f1.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2?useUnicode=true&characterEncoding=utf8&autoReconnect=true&failOverReadOnly=false&maxReconnects=100', table='judgment_zhangye_400w_v3_03',properties={"user": "weiwc", "password": "HHly2017."})
