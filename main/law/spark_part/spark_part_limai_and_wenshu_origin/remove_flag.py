...
cursor = conn.cursor()
sql = 'select id,doc_content from adjudication where id<3 '
cursor.execute(sql)
row_2 = cursor.fetchall()
print "read mysql success... %s" % time.asctime( time.localtime(time.time()) )
...
for row in row_2 :
    ...
    if s1:
        ...
        sql2 = " update tmp_weiwc set lawlist='" + ",".join(s1) + "' where id= '" + str(row[0]) + "'"
        cursor.execute(sql2)
        # print ",".join(s1)
    conn.commit()
print "execute success... %s" %  time.asctime( time.localtime(time.time()) )
cursor.close()
conn.close()