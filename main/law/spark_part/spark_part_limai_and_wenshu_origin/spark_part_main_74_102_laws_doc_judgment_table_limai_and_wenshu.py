...
def format_content(content,doc_from):
    ...
    if doc_from != "wenshu-gov":
        ...
        content_text = re.sub(reg_link, "", content_text)    #删除a标签，是否可注释，没有亲自测试
...
def doc_items(items):
    ...
    uuid = items[1]
    doc_content = items[2]
    # id, uuid, doc_content, caseid, title, court_origin, lawlist, casedate_origin, reason_type, type, judge_type, doc_from
    caseid = items[3]
    title = items[4]
    court = items[5]
    lawlist = items[6]
...
# ,uuid_,caseid,title,court,lawlist,reason_type,type,judge_type..........uuid_,caseid,title,court,lawlist,casedate,reason_type,type,judge_type
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.74.102:3306/laws_doc', table='(select id,uuid,doc_content,caseid,title,court_origin,lawlist,casedate_origin,reason_type,type,judge_type,doc_from from judgment  where is_crawl = 1) tmp',column='id',lowerBound=1,upperBound=3861714,numPartitions=99,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_judgment?useUnicode=true&characterEncoding=utf8', table='judgment_etl_v2',properties={"user": "weiwc", "password": "HHly2017."})
