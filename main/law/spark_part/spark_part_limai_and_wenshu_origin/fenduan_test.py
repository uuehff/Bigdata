...
def doc_items(items, b_value):
    ...
    items_dict = dict(uuid=uuid,party_info=party_info, trial_process=trial_process, trial_request=trial_request,
...
if __name__ == "__main__":
    ...
    conn = pymysql.connect(host='10.41.104.5', user='root', passwd='Hhly@2018', db='laws_doc', charset='utf8')
    cursor = conn.cursor()
    sql = 'select id,uuid,doc_content from judgment where uuid in ("bf3262c4-cb39-4282-8455-a835012d9432","18983fe0-f5dd-4990-9aa9-a8b300d00360","af859d1c-42ad-4ff6-9bab-a81200a1724e","4611bcbb-f5c2-4970-9ceb-a8a1009cda29") '  # LOCATE函数，包含||,返回大于0的数值。
    cursor.execute(sql)
    row_2 = cursor.fetchall()

    # df = sqlContext.read.jdbc(url='jdbc:mysql://10.41.104.5:3306/laws_doc', table='(select id,uuid,doc_content from judgment where id > 5816521 ) tmp',column='id',lowerBound=5816521,upperBound=6532764,numPartitions=24,properties={"user": "root", "password": "Hhly@2018"})
