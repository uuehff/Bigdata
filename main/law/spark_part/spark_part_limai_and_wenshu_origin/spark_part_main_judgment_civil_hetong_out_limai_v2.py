...
def doc_items(items):
    ...
    uuid = items[1]
    doc_content = items[2]
    # id, uuid, doc_content, caseid, title, court, lawlist, casedate, reason_type, type, judge_type
    caseid = items[3]
    title = items[4]
    court = items[5]
    lawlist = items[6]
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.10.22:3306/laws_doc', table="(select id,uuid,doc_content,caseid,title,court,lawlist,casedate,reason_type,type,judge_type from judgment where is_crawl = 1 ) tmp ",column='id',lowerBound=1,upperBound=6003507,numPartitions=99,properties={"user": "tzp", "password": "123456"})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_new?useUnicode=true&characterEncoding=utf8', table='judgment_new_v3',properties={"user": "weiwc", "password": "HHly2017."})
