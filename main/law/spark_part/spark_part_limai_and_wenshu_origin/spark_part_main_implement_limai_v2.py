...
def doc_items(items):
    ...
    court_find = pymysql.escape_string(court_find).replace("\\n","\n").replace(r'''\\r''', "").replace("\\r", "")  #r'''\\r''替换文中的\\r字符串，"\\r"替换文中\r字符串，等价于r'''\r''
...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.10.22:3306/laws_doc', table='(select id,uuid,doc_content,caseid,title,court,lawlist,reason_type,type,judge_type from implement ) tmp',column='id',lowerBound=1,upperBound=2119611,numPartitions=99,properties={"user": "tzp", "password": "123456"})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_implement?useUnicode=true&characterEncoding=utf8', table='implement_civil_etl_v3',properties={"user": "weiwc", "password": "HHly2017."})
