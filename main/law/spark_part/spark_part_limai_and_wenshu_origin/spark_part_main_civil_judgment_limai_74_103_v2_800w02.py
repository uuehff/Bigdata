...
def doc_items(items):
    ...
    uuid = items[1]
    doc_content = items[2]
    # id, uuid, doc_content, caseid, title, court_origin, lawlist, casedate_origin, type, judge_type
    caseid = items[3]
    title = items[4]
    court = items[5]
    lawlist = items[6]
    ...
    if judge_type == u"判决" or judge_type == u"裁定" or judge_type == u"调解" or judge_type == u"通知" or judge_type == u"其他" :
        if judge_type == u"其他":  #其他大部分是以调解方式结案的，不公开文书内容，也有部分是判决书
            judge_type = u"判决"
    else:
        return None
    ...
    # 无标签下的数据处理无法分段,不管是判决、还是通知，都放入court_idea字段,以下两段的处理都是为了得到完整的court_idea值。
    if court_idea_all == "":
    ...
    # ,uuid_,caseid,title,court,lawlist,casedate,reason_type,type,judge_type

    return (id,uuid,party_info,trial_process,trial_request,court_find,court_idea,judge_result,doc_footer,uuid_,caseid,title,court,lawlist,casedate,reason_type,type,judge_type)
...
if __name__ == "__main__":
    ...
    # df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.10.22:3306/laws_doc', table='(select id,uuid,doc_content from adjudication_civil where id  >= 1010 and id <= 1041 and doc_from = "limai" ) tmp',column='id',lowerBound=1,upperBound=1800000,numPartitions=1,properties={"user": "tzp", "password": "123456"})16437624
    df = sqlContext.read.jdbc(url='jdbc:mysql://192.168.74.103:3306/civil', table="(select id,uuid,doc_content,caseid,title,court_origin,lawlist,casedate_origin,type,judge_type from judgment where id > 8000000 and is_crawl = 1 ) tmp ",column='id',lowerBound=8000000,upperBound=16437624,numPartitions=99,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave3:3306/laws_doc_civil?useUnicode=true&characterEncoding=utf8', table='civil_etl_v2_800w_02',properties={"user": "weiwc", "password": "HHly2017."})
