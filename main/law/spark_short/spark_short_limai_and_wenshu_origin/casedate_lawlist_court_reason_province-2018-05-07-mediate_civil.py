...
if __name__ == "__main__":
    ...
    def get_reason(x):
        ...
        return (x[1],("||".join(list(set(name))),reason_uids,casedate,plt_claim, dft_rep, crs_exm))
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_mediate',
                               table='(select id,uuid,court,title,trial_request,trial_process,court_find,doc_footer from mediate_civil_etl  ) tmp',
                               column='id', lowerBound=1, upperBound=131608, numPartitions=2,
                               properties={"user": "weiwc", "password": "HHly2017."})
    # court:4778条
    df1 = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc',
    ...
    result = uuid_reason.join(uuid_province_full_uid).map(lambda x:(x[0],x[1][0][0],x[1][0][1],x[1][0][2],x[1][0][3],x[1][0][4],x[1][0][5],x[1][1][0],x[1][1][1]))
    ...
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_mediate?useUnicode=true&characterEncoding=utf8', table='mediate_civil_other_fields',properties={"user": "weiwc", "password": "HHly2017."})
