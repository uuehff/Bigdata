...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_lawyers', table='(select id,lawyer,law_office from lawyers) tmp',column='id',lowerBound=1,upperBound=715185,numPartitions=20,properties={"user": "weiwc", "password": "HHly2017."})  #8606165
    # df2 = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2', table='(select id,uuid,party_info,court_idea,judge_type from judgment_zhangye_xingzheng_v2_result  ) tmp2',column='id',lowerBound=1,upperBound=8606165,numPartitions=79,properties={"user": "weiwc", "password": "HHly2017."})
    df2 = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2', table='(select id,uuid,party_info,court_idea,judge_type from judgment_zhangye_xingzheng_v3_result  ) tmp2',column='id',lowerBound=1,upperBound=8606165,numPartitions=79,properties={"user": "weiwc", "password": "HHly2017."})


    def filter_valid(x):
        if x[1] and x[2] and x[1] != "" and x[2] != "":
            return True
        else:
            return False
    ...
    all_result = lawyer_law_office.leftOuterJoin(id_result).map(lambda x:get_all_result(x)).filter(lambda x:filter_valid(x))
    ...
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2?useUnicode=true&characterEncoding=utf8', table='judgment_zhangye_xingzheng_v2_v3_result_lawyer_id',properties={"user": "weiwc", "password": "HHly2017."})
