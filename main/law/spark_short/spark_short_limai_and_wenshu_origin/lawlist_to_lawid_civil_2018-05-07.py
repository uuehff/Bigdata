...
def get_lawlist_ids(ids):
    lawlist_id = []
    for x in ids:
        lawlist_id.append(x)
    return "||".join(lawlist_id)
...
if __name__ == "__main__":
    ...
    lawlist= sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave1:3306/civil', table='(select id,uuid,lawlist from judgment_civil_all ) tmp2',column='id',lowerBound=1,upperBound=11295123,numPartitions=78,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    # p2 = ur'[\u300a\u300b]'  # 按《》切分
    c = lawlist.select('uuid','lawlist').map(lambda x:(x[0],x[1])).filter(lambda x:filter_(x)).flatMapValues(lambda x: x.split("||")).filter(filter_).map(lambda x: (x[1], x[0])).groupByKey().mapValues(lambda v: get_uuids(v))
    ...
    lawlist_title_id_result = lawlist_id2.join(c).map(lambda x:x[1]).flatMapValues(lambda x:(x.split("||"))).\
        map(lambda x:(x[1],x[0])).groupByKey().mapValues(lambda x:(get_lawlist_ids(x)))   #uuid|vs
    ...
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh-slave1:3306/civil?useUnicode=true&characterEncoding=utf8', table='law_id_civil_0001',properties={"user": "weiwc", "password": "HHly2017."})
