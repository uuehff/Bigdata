...
if __name__ == "__main__":
    ...
    # judgment_new：556万
    df = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_new',
                               table='(select id,uuid,court,title,trial_process from judgment_new  ) tmp',
                               column='id', lowerBound=1, upperBound=5560000, numPartitions=20,
    ...
    uuid_province_full_uid = uuid_court.join(court_province_full_uid).map(lambda x:x[1])
    ...
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_new?useUnicode=true&characterEncoding=utf8', table='zzz',properties={"user": "root", "password": "root"})
