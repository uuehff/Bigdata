...
if __name__ == "__main__":
    ...
    lawlist_id = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave2:3306/law', table='(select id,title_short,art_num,lawlist_id from law_rule_result2) tmp',column='id',lowerBound=1,upperBound=2650000,numPartitions=30,properties={"user": "root", "password": "HHly2017."})
    ...
    lawlist= sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_new', table='(select id,uuid,lawlist_origin from tmp_weiwc where id < 100) tmp2',column='id',lowerBound=1,upperBound=5600000,numPartitions=30,properties={"user": "root", "password": "root"})
    ...
    p2 = ur'[\u300a\u300b]'  # 按《》切分
    c = lawlist.select('uuid','lawlist_origin').map(lambda x:(x[0],x[1])).flatMapValues(lambda x: etl_lawlist(p1, p2, x)).filter(filter_).map(lambda x: (x[1], x[0])).groupByKey().mapValues(lambda v: get_uuids(v))
    ...
    schema = StructType([StructField("uuid", StringType(), False),StructField("law_id", StringType(), False)])

    f = sqlContext.createDataFrame(lawlist_title_id_result, schema=schema)
    # , mode = "overwrite"
    # useUnicode = true & characterEncoding = utf8，指定写入mysql时的数据编码，否则会乱码。
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh-slave1:3306/laws_doc_new?useUnicode=true&characterEncoding=utf8', table='uuid_law_id',properties={"user": "root", "password": "root"})
