...
if __name__ == "__main__":
    ...
    lawlist_id = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_new', table='(select * from judgment_new_other_fields_v2) tmp',column='id',lowerBound=1,upperBound=6003072,numPartitions=69,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    uuid_uuid= sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_new', table='(select id,uuid,uuid_ from judgment_new_v2 ) tmp2',column='id',lowerBound=1,upperBound=6003507,numPartitions=30,properties={"user": "weiwc", "password": "HHly2017."})


    a = uuid_uuid.map(lambda x:(x[1],x[2]))  #uuid,uuid_

    # id,    uuid,    reason,    reason_uid,    province,    court_uid

    b = lawlist_id.map(lambda x:(x[1],x))

    result = a.join(b).map(lambda x:x[1]).map(lambda x:(x[1][0],x[0],x[1][2],x[1][3],x[1][4],x[1][5]))
    ...
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_new?useUnicode=true&characterEncoding=utf8', table='judgment_new_other_fields_v2_result',properties={"user": "weiwc", "password": "HHly2017."})
