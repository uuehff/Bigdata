...
if __name__ == "__main__":
    ...
    df = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2',
                               table='(select id,uuid,court,title,trial_request,trial_process,court_find,doc_footer,court_idea,casedate from judgment_zhangye_civil_v5_result where id >= 15000000  ) tmp',
                               column='id', lowerBound=15000000, upperBound=21887040, numPartitions=60,
                               properties={"user": "weiwc", "password": "HHly2017."})
    # court:4778条
    ...
    def get_result(x):
        if x[1][1] is None:
            return (x[0], x[1][0][0], x[1][0][1], x[1][0][2], None, None)
        else:
    ...
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2?useUnicode=true&characterEncoding=utf8', table='judgment_zhangye_civil_v5_other_fields',properties={"user": "weiwc", "password": "HHly2017."})
