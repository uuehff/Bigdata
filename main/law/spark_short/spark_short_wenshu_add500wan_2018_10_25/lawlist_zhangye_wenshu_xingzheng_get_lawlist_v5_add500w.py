...
if __name__ == "__main__":
    ...
    lawlist_id = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave3:3306/law',
                                      ...
                                      column='id', lowerBound=1, upperBound=2881160, numPartitions=20,
    ...
    lawlist = sqlContext.read.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2',
                               table='(select id,uuid,court_idea from judgment_zhangye_xingzheng_v5_result ) tmp',
                               column='id', lowerBound=207051, upperBound=19848983, numPartitions=20,properties={"user": "weiwc", "password": "HHly2017."})
    ...
    def get_list(x):
        ...
        for i in x:
            l.append(i)
        return l
    # c = tmp_data.mapValues(lambda x: etl_lawlist(p1, p2, x)).flatMapValues(lambda x:get_list(x))
    ...
    # lawlist_title_id_result.foreach(p)
    ...
    # print str(f.count()) + "======================"
    f.write.jdbc(url='jdbc:mysql://cdh5-slave2:3306/laws_doc_zhangye_v2?useUnicode=true&characterEncoding=utf8', table='uuid_law_id_zhangye_xingzheng_v5',properties={"user": "weiwc", "password": "HHly2017."})
