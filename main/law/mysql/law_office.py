...
for row in row_2 :
    ...
    if row[1] and row[1] != "":
        s = row[1].split("||")
        for i in s:
            sql2 = 'select law_office from lawyers where id = ' + i
            cursor2.execute(sql2)
            law_office.append(cursor2.fetchone()[0])

        sql3 = " update tmp_lawyers set law_office='" + "||".join(law_office) + "' where uuid= '" + row[0] + "'"
