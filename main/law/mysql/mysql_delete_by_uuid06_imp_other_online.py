...
for row in row_2 :
    c += 1
    # sql = " delete from imp_other_etl_v2 where uuid = '" + row[0] + "' "
    # cursor.execute(sql)
    # sql = " delete from imp_other_etl_v2_court_cate_judge_footer where uuid = '" + row[0] + "' "
    # cursor.execute(sql)
    # sql = " delete from imp_other_etl_v2_field where uuid = '" + row[0] + "' "
    # cursor.execute(sql)
    # sql = " delete from imp_other_etl_v2_lawyer where uuid = '" + row[0] + "' "
    # cursor.execute(sql)
    # sql = " delete from imp_other_etl_v2_organization where uuid = '" + row[0] + "' "
    # cursor.execute(sql)
    sql = " delete from imp_other_uuid_law_id_v2 where uuid = '" + row[0] + "' "
    cursor.execute(sql)
    sql = " delete from imp_other_other_fields_v2 where uuid = '" + row[0] + "' "
    cursor.execute(sql)
print c
conn.commit()
cursor.close()
