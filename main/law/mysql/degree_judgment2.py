...
cursor = conn.cursor()
sql = 'select id,punish_cate,punish_date,if_delay,delay_date from judgment2_visualization_v2 where id = 31203 '   #LOCATE函数，包含||,返回大于0的数值。
cursor.execute(sql)
...
for row in row_2 :
        ...
        sql2 = " update judgment2_visualization_v2 set degree='" + degree + "' where id = '" + str(row[0]) + "'"
        cursor.execute(sql2)
conn.commit()
cursor.close()
conn.close()