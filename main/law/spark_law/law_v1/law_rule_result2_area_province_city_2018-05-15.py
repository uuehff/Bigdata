...
if __name__ == "__main__":
    ...
    lawyer_info = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave2:3306/law', table='(select id,province,city,full_uid from province_city_full_uid) tmp',column='id',lowerBound=1,upperBound=501,numPartitions=1,properties={"user": "root", "password": "HHly2017."})
    lawyers = sqlContext.read.jdbc(url='jdbc:mysql://cdh-slave2:3306/law', table='(select id,law_id,area,title_short from law_rule_result_article) tmp2',column='id',lowerBound=1,upperBound=609664,numPartitions=23,properties={"user": "root", "password": "HHly2017."})
    ...
    f.write.jdbc(url='jdbc:mysql://cdh-slave2:3306/law?useUnicode=true&characterEncoding=utf8', table='law_area_uid',properties={"user": "root", "password": "HHly2017."})
