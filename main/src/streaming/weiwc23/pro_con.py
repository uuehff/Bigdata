...
#生产kafka数据，通过字符串形式
def produce_kafka_data(kafka_topic, msg, max_request_size=100000000):
    with kafka_topic.get_sync_producer(max_request_size=100048576) as producer:
...
for word in uni_words:
    ...
    if inverted_data and len(inverted_data.get('value')) < 20:
        inverted_list.append(inverted_data)

produce_kafka_data(topic_in, inverted_list, max_request_size=100000000)
